...
def __loadConfig():
    ...
    # the default config is also always loaded to try and fall back to its values if sections or keys
    # are missing from the user config
...
def get(section, option, type = str):
    ...
    if __CONFIG is None:
        __loadConfig()
    if not __CONFIG.has_section(section) or not __CONFIG.has_option(section, option):
        # value is not preset in user config, try and return it from the default one
